...
def run_vision_extraction_pipeline(user_id: str, new_image_keys: list):
    ...
    try:
        # Download images from S3 into memory
        ...
        # Invoke Vision Agent using the new SDK
        ...
        print(f"✅ [VISION AGENT] Extracted {len(extracted_data.get('ingredients', []))} ingredients. Inferred type: {extracted_data.get('inferred_product_type')}")
...
async def invoke_agent(payload: ChatPayload):
    ...
    try:
        ...
        print(f"⚠️ Pipeline check failed, proceeding to chat: {e}")

    # --- PIPELINE STEP 2: Consultation Agent ---
